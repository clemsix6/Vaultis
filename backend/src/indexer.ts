...
function parseLog(log: Log, address: `0x${string}` | undefined, contractName: string): ParsedEvent | null {
  if (!address || log.address.toLowerCase() !== address.toLowerCase()) return null;

  const topic0 = log.topics[0];
  if (!topic0) return null;

  // KYC events (single indexed address)
  const kycEventMap: Record<string, string> = {
    // Whitelisted(address indexed account)
    "0xaab7954e9d246b167ef88aeddad35209ca2489d95a8aeb59e288d9b19fae165a": "Whitelisted",
    // RemovedFromWhitelist(address indexed account)
    "0x1747b48b6ade85d7dc97c0f523e0e780795930a468c01b18a51546791fdd3ac6": "RemovedFromWhitelist",
    // Blacklisted(address indexed account)
    "0xffa4e6181777692565cf28528fc880fd7a9ccd244e05194e0e10e53600c2e5e7": "Blacklisted",
    // RemovedFromBlacklist(address indexed account)
    "0x2b6bf71b58b3583add364b3d9060ebf8019650f4f6691a96d749ea85d03329c9": "RemovedFromBlacklist",
  };

  // We won't rely on topic hashes (they can vary). Instead, let's use getLogs per event.
  // This function is a fallback — the main logic uses getLogs with specific events.
  return null;
}
...
async function indexEvents(fromBlock: bigint, toBlock: bigint): Promise<number> {
  ...
  // We'll batch-fetch timestamps for each block we encounter
