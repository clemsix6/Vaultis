...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    // Rate limit: 3 registrations per hour per wallet
 ...
 *
 * Verifies the email and whitelists the wallet on-chain.
 */
app.get("/api/auth/verify", async (req, res) => {
  try {
    ...
    try {
      ...
      // Email is verified, but on-chain failed — admin can retry manually
      res.json({
 ...
 *
 * Returns the KYC status for a wallet.
 */
...
/**
 * GET /api/events?type=Swap&limit=50&offset=0
 *
 * Returns blockchain events, optionally filtered by type.
 */
 ...
 *
 * Returns swap/trade events from the DEX pool.
 */
 ...
 *
 * Returns a feed of the latest blockchain events across all contracts.
 */
